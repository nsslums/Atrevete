	...
	const options = {
		renderNode: {
			...
			[BLOCKS.EMBEDDED_ENTRY]: ({ data }) => {
				let link;
				if (data.target.__typename == "ContentfulEvent") link = `/event/${GetSlug(data.target)}`;
				else if (data.target.__typename == "ContentfulPost") link = `/post/${GetSlug(data.target)}`;
				else return null;
				return (
					<div>
			...
			[INLINES.EMBEDDED_ENTRY]: ({ data }) => {
				...
				return <Link to={link}>{data.target.title}</Link>;
			},
		},
	};
			...
			<div css={Wrap}>
				<div css={[block]}>
					{data.contentfulEvent.eye_catch ? (
						<EventHead
							title={title}
							imageURL="src/images/event.jpg"
							date={data.contentfulEvent.date}
							GatsbyImageData={data.contentfulEvent.eye_catch.gatsbyImageData}
						/>
					) : (
						<EventHead
							title={title}
				...
				<div css={[block]}>
					<Head2 text="概要" />
					<div css={textStyle}>
						{!data.contentfulEvent.overview
							? false
							: renderRichText(data.contentfulEvent.overview, options)}
					</div>
				</div>
				<div css={[block]}>
					<Head2 text="募集要件" />
					<div css={textStyle}>
						{!data.contentfulEvent.requirements
							? false
							: renderRichText(data.contentfulEvent.requirements, options)}
					</div>
				</div>
				<div css={horizon}>
				...
				<div css={[block, { margin: "1.4em auto" }]}>
					<EventFormCard
						start_date={start_reception}
						end_date={end_reception}
						onClick={navigation}
					/>
				</div>
...
export const query = graphql`
	query ($contentful_id: String!) {
		...
		contentfulEvent(contentful_id: { eq: $contentful_id }, hidden: { ne: true }) {
			title
