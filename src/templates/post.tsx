	...
	const options = {
		renderNode: {
			[BLOCKS.PARAGRAPH]: (node: any, children: any) => {
				...
				return <p dangerouslySetInnerHTML={{ __html: node.content[0].value }}></p>;
			},
			...
			<div css={[block, { maxWidth: 770 }]}>
				{data.contentfulPost.eye_catch ? (
					<PostHead
						...
						date={data.contentfulPost.createdAt}
						GatsbyImageData={data.contentfulPost.eye_catch.gatsbyImageData}
						tags={data.contentfulPost.metadata.tags || []}
					/>
				) : (
					<PostHead
						title={data.contentfulPost.title}
				...
				<div css={contentStyle}>
					{!data.contentfulPost.content
						? false
						: renderRichText(data.contentfulPost.content, options)}
				</div>
								...
								event.eye_catch ? (
									<div key={event.contentful_id}>
										<EventCard
											...
											image={event.eye_catch.gatsbyImageData}
										/>
									</div>
								) : (
									<div key={event.contentful_id}>
...
export const query = graphql`
	query ($contentful_id: String!) {
		...
		contentfulPost(contentful_id: { eq: $contentful_id }, hidden: { ne: true }) {
			title
			slug
